...
/// with a newer version surfaces as its own plan line, alongside the dependent's rebuild (the
/// dependent's address folds the dep's current rune, so bumping the dep drifts the dependent).
#[test]
fn dry_run_upgrade_names_pulled_in_dependency() {
    ...
    // A newer lib: upgrading app must pull lib up and rebuild app against it.
